...
def generate_mcqs(llm, relevant_chunks, topic, num_mcqs, difficulty):
    """
    ...
    Args:
        ...
        difficulty (str): Difficulty level of MCQs
    ...
    system_message = ChatMessage(role="system", content=f"""You are an expert educator specializing in creating {difficulty.lower()} difficulty MCQs.
    For each MCQ, provide:
    1. A {difficulty.lower()} complexity question
    2. Four options labeled A) through D)
    3. The correct answer
    
    Difficulty levels:
    - Easy: Simple, straightforward questions testing basic recall and understanding
    - Medium: Questions requiring some interpretation and moderate reasoning
    - Hard: Complex questions involving deep analysis, multiple concepts, or critical thinking
    ...
    Based on the following context, generate {num_mcqs} multiple-choice questions (MCQs) on the topic '{topic}' with {difficulty.lower()} difficulty:
    Context: {context}
    
    Generate exactly {num_mcqs} MCQs in the specified JSON-like format, separated by ###.
    Ensure each MCQ matches the {difficulty.lower()} difficulty level.
...
# Existing functions remain the same: parse_mcqs, process_pdf_for_mcqs, etc.

def process_pdf_for_mcqs(pdf_path, query, topic, num_mcqs, difficulty):
    """
    ...
    mcqs_text = generate_mcqs(llm, relevant_chunks, topic, num_mcqs, difficulty)
...
def main():
    ...
    if uploaded_file and generate_button:
        with st.spinner(f"Generating {num_mcqs} {difficulty} MCQs... Please wait."):
            try:
                # Save uploaded file
                ...
                mcqs = process_pdf_for_mcqs(pdf_path, query, topic, num_mcqs, difficulty)
                
                # Display results
                st.subheader(f"Generated {len(mcqs)} {difficulty} MCQs for Topic: *{topic}*")
